...
public final class DefaultWeaponService implements WeaponService {
	...
	@Override
	public final Iterable<WeaponAdjustment> getAdjustment(Long id) {
		final Optional<? extends Weapon> weapon;
		...
		return levels.stream().map(this::toWeaponAdjustment).collect(Collectors.toList());
	}
	...
	private final WeaponProgression toWeaponProgression(final Iterable<WeaponLevelNode> levelNodes,
		...
		pathNames = StreamSupport.stream(levelNodes.spliterator(), false).map(WeaponLevelNode::getPath).distinct()
				.sorted().collect(Collectors.toList());

		paths = new ArrayList<>();
		for (final String pathName : pathNames) {
			currentLevels = StreamSupport.stream(levels.spliterator(), false)
					.filter((l) -> pathName.equals(l.getPath())).map((l) -> toWeaponProgressionLevel(l, levelNodes))
		...
		name = StreamSupport.stream(levels.spliterator(), false).map(WeaponLevel::getName).findAny().orElse("");
	...
	private final WeaponProgressionLevel toWeaponProgressionLevel(final WeaponLevel level,
		...
		levelNodeFound = StreamSupport.stream(levelNodes.spliterator(), false)
				.filter(n -> n.getName().equals(level.getName()) && n.getPath().equals(level.getPath())
						&& n.getLevel().equals(level.getLevel()))
				.findFirst();
