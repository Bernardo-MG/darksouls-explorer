...
public interface WeaponLevelRepository extends JpaRepository<PersistentWeaponLevel, Long> {

    @Query("SELECT l FROM WeaponLevel l WHERE l.name = :name ORDER BY l.path ASC, l.level ASC")
    public Collection<PersistentWeaponLevel> findAllByName(@Param("name") final String name);
