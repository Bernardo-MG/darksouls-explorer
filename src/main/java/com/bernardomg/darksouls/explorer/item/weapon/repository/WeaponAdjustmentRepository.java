...
public interface WeaponAdjustmentRepository extends JpaRepository<PersistentWeaponAdjustment, Long> {

    @Query("SELECT a FROM WeaponAdjustment a WHERE a.name = :name ORDER BY a.faith ASC, a.intelligence ASC")
    public Collection<PersistentWeaponAdjustment> findAllByName(@Param("name") final String name);
